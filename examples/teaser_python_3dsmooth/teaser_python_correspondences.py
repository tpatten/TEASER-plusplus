...
def pair_eval_helper(scene_path, desc_path, gt_mat, frag1_idx, frag2_idx):
    ...
    print(frag1_desc.shape) # (5000, 32)
    ...
    print(frag2_desc.shape) # (5000, 32)
    ...
    print(frag1_pc) # geometry::PointCloud with 347060 points
    print(frag2_pc) # geometry::PointCloud with 403148 points
    ...
    print(frag1_indices.shape) # (5000,)
    print(frag2_indices.shape) # (5000,)
    ...
    print(frag1_pc_keypoints.shape)  # (5000, 3) => point coordinates
    print(frag2_pc_keypoints.shape)  # (5000, 3) => point coordinates
    ...
    print(ref_matched_key.shape) # (3, 840)
    print(test_matched_key.shape) # (3, 840)
...
def pair_eval_helper_fpfh(scene_path, desc_path, gt_mat, frag1_idx, frag2_idx):
    # Load point clouds
    fragment1_name = "cloud_bin_" + str(frag1_idx)
    fragment2_name = "cloud_bin_" + str(frag2_idx)
    frag1_pc = o3d.io.read_point_cloud(
    ...
    ## Downsample
    #frag1_pc = frag1_pc.voxel_down_sample(voxel_size=0.05)
    #frag2_pc = frag2_pc.voxel_down_sample(voxel_size=0.05)
    #frag1_sz = np.asarray(frag1_pc.points).shape[0]
    #frag2_sz = np.asarray(frag2_pc.points).shape[0]
    #if frag1_sz < frag2_sz:
    #    frag2_pc.points = o3d.utility.Vector3dVector(np.asarray(frag2_pc.points)[0:frag1_sz, :])
    #elif frag2_sz < frag1_sz:
    #    frag1_pc.points = o3d.utility.Vector3dVector(np.asarray(frag1_pc.points)[0:frag2_sz, :])

    voxel_size = 0.05
    frag1_pc, frag1_fpfh = preprocess_point_cloud(frag1_pc, voxel_size)
    frag2_pc, frag2_fpfh = preprocess_point_cloud(frag2_pc, voxel_size)
    frag1_fpfh.data = np.asarray(frag1_fpfh.data).T
    frag2_fpfh.data = np.asarray(frag2_fpfh.data).T
    frag1_sz = np.asarray(frag1_pc.points).shape[0]
    frag2_sz = np.asarray(frag2_pc.points).shape[0]
    if frag1_sz < frag2_sz:
        frag2_pc.points = o3d.utility.Vector3dVector(np.asarray(frag2_pc.points)[0:frag1_sz, :])
        frag2_fpfh.data = np.asarray(frag2_fpfh.data)[0:frag1_sz, :]
    elif frag2_sz < frag1_sz:
        frag1_pc.points = o3d.utility.Vector3dVector(np.asarray(frag1_pc.points)[0:frag2_sz, :])
        frag1_fpfh.data = np.asarray(frag1_fpfh.data)[0:frag2_sz, :]
    print(np.asarray(frag1_fpfh.data).shape) # (6147, 33)
    print(np.asarray(frag2_fpfh.data).shape) # (6147, 33)

    frag1_key = o3d.geometry.PointCloud()
    frag1_key.points = o3d.utility.Vector3dVector(np.asarray(frag1_pc.points))

    frag2_key = o3d.geometry.PointCloud()
    frag2_key.points = o3d.utility.Vector3dVector(np.asarray(frag2_pc.points))

    ref_matched_key, test_matched_key = find_mutually_nn_keypoints(frag2_key, frag1_key, frag2_fpfh, frag1_fpfh)
    ref_matched_key = np.squeeze(ref_matched_key)
    test_matched_key = np.squeeze(test_matched_key)

    print(ref_matched_key.shape) # (3, 3)
    print(test_matched_key.shape) # (3, 3)

	# Transpose
    frag1_pts = np.transpose(np.asarray(frag1_pc.points))
    frag2_pts = np.transpose(np.asarray(frag2_pc.points))
    print(frag1_pts.shape, frag2_pts.shape)

    est_mat, max_clique, time = execute_teaser_global_registration(frag1_pts, frag2_pts)

    if VISUALIZE:
        gt_inliers = bench_utils.get_gt_inliers(test_matched_key.T, ref_matched_key.T, gt_mat)
        print("gt_inliers below:")
        print(gt_inliers)
        ...
        print("Now drawing registration results ...")
        draw_registration_result(test_matched_key.T, ref_matched_key.T, frag1_pc, frag2_pc, est_mat, max_clique, gt=gt_mat, gt_inliers=gt_inliers)
...
def pair_eval_helper_no_correspondences(scene_path, desc_path, gt_mat, frag1_idx, frag2_idx):
    # Load point clouds
    fragment1_name = "cloud_bin_" + str(frag1_idx)
    fragment2_name = "cloud_bin_" + str(frag2_idx)
    frag1_pc = o3d.io.read_point_cloud(
    ...
    if VISUALIZE:
        print("Now drawing registration results ...")
        draw_registration_result(None, None, frag1_pc, frag2_pc, est_mat, None, gt=gt_mat, gt_inliers=None)
...
if __name__ == "__main__":
    ...
    #est_mat = pair_eval_helper(scene_path, desc_path, gt_mat, pair[0], pair[1])
    est_mat = pair_eval_helper_no_correspondences(scene_path, desc_path, gt_mat, pair[0], pair[1])
